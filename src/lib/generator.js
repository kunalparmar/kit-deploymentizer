...
class Generator {
  ...
  _createLocalConfiguration(config, resourceName, resource) {
    ...
    if (resource.env) {
      const env = resourceHandler.mergeEnvs(localConfig.env, resource.env);
  ...
  processResource(resource, resourceName, fileStats) {
    ...
    return Promise.try( () => {
      ...
      return this._createLocalConfiguration(this.options.clusterDef.configuration(), resourceName, resource);
    }).then( (localConfig) => {
      // render the service template
      _localConfig = localConfig;
      return this.processService(resource, localConfig);
    }).then( () => {
      ...
      if (this.options.save === true) {
        return yamlHandler.saveResourceFile(this.options.exportPath, fileStats.name, resourceYaml);
      } else {
